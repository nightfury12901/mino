  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/panel/login', {
        ...
        body: JSON.stringify({ password, scope: 'attendance' })
      });
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <Card className="w-full max-w-sm bg-slate-900 border-slate-800">
        <CardHeader className="text-center space-y-2">
          ...
          <CardTitle className="text-2xl font-bold text-white tracking-tight">Scanner Login</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleLogin} className="space-y-4">
            <Input 
              ...
              autoFocus
            />
            <Button type="submit" className="w-full h-12 bg-cyan-600 hover:bg-cyan-500 text-white font-bold" disabled={loading || !password}>
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Enter Scanner Mode'}
            </Button>
          </form>
        </CardContent>
      </Card>
